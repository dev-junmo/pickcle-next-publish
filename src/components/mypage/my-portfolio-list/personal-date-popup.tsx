...
import { businessDateStore, personalDateStore } from 'src/status/store';
...
export default function PersonalDatePopup({ triggerChildren, children, ...props }: Props) {
  ...
  return (
    <div>
      <RadixDialog.Root {...props} open={personalDate} onOpenChange={personalDateState}>
        ...
        <RadixDialog.Portal>
          ...
          <RadixDialog.Content className='no-scrollbar fixed bottom-0 left-0 z-[101] h-[70%] w-full overflow-x-scroll rounded-tl-[30px] rounded-tr-[30px] bg-white p-[30px] text-center sm:p-[50px] md:left-1/2 md:top-1/2 md:h-auto md:w-[610px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[10px]'>
            ...
            <div className='mt-[50px] sm:pl-[50px]'>
            ...
            <div className='mt-[50px] flex items-center sm:pl-[50px]'>
              ...
              {modifyAction ? (
                ...
                <span className='text-[18px] font-[500] '>타사이트 폴로</span>
              )}
            </div>
            ...
            <p className='my-[50px] text-[14px] font-[500] text-[#2d2d2d]'>
              <span className='text-[#8586db]'>제목/협찬방식/날짜/내용</span> 외에는 수정이
              불가하오니, 변경을 <br />
              원하시는 경우 삭제 후 재 등록해 주시길 바랍니다.
            </p>
            <div className='flex items-center justify-center gap-[56px]'>
              {!modifyAction && (
                <button className='h-[49px] w-[136px] rounded-[10px] bg-[#66676e] text-[20px] font-bold text-[#fff]'>
                  삭제
                </button>
              )}
              {!modifyAction && (
              ...
              <button
                ...
                onClick={() => (modifyAction ? setModifyAction(false) : personalDateState(false))}
              >
                확인
              </button>
            </div>
